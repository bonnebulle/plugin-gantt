    ...
    for (var y in dates) {
        for (var m in dates[y]) {
            ...
            for (var d in dates[y][m]) {
                // Bonnebulle add first day of month ( need optimisation )
                let dday = dates[y][m][d].getDate();
                if (dday == "1") {
                    if (this.options.showToday && this.isToday(dates[y][m][d])) {
                        daysDiv.append(jQuery("<div>", { "class": "ganttview-hzheader-day" }).addClass("today").addClass("onefirst").append(dates[y][m][d].getDate()));
                    } else {
                        daysDiv.append(jQuery("<div>", { "class": "ganttview-hzheader-day" }).addClass("onefirst").append(dates[y][m][d].getDate()));
                    }
                } else if (this.options.showToday && this.isToday(dates[y][m][d])) {
                    daysDiv.append(jQuery("<div>", { "class": "ganttview-hzheader-day" }).addClass("today").append(dates[y][m][d].getDate()));
                } else {
                    daysDiv.append(jQuery("<div>", { "class": "ganttview-hzheader-day" }).append(dates[y][m][d].getDate()));
                }
                
                if (this.options.showToday && this.isToday(dates[y][m][d])) {
                    daysDiv.addClass("today");
                }
            }
        }
    }
...
Gantt.prototype.addTaskBarText = function(container, record, size) {
    if (size >= 4) {
        container.html($('<span>').text(record.title));
        // record.progress + ' - #' + record.id + ' ' + 
    }
    else if (size >= 2) {
        container.html($('<span>').text(record.title));
    }
...
Gantt.prototype.updateDataAndPosition = function (block, startDate) {
    ...
    // Set new start date
    // BonneBulle QFix "+ 1" décalage entre dragndrop et reload page (storage date)
